...
vec3 RotateVectorAboutY(float angle, vec3 vec)
{
    ...
    mat3x3 rotationMatrix = {
      vec3(cos(angle), 0.0, sin(angle)), vec3(0.0, 1.0, 0.0), vec3(-sin(angle), 0.0, cos(angle))};
...
vec3 Lighting(vec3 F0)
{
    ...
    {
        vec3 Li        = -u_Lights.Direction;
        vec3 Lradiance = u_Lights.Radiance;
...
vec3 IBL(vec3 F0, vec3 Lr)
{
    ...
    if (u_RadiancePrefilter > 0.5)
        specularIrradiance =
          PrefilterEnvMap(m_Params.Roughness * m_Params.Roughness, R) * u_RadiancePrefilter;
    else
        specularIrradiance = textureLod(u_EnvRadianceTex,
                                        RotateVectorAboutY(u_EnvMapRotation, Lr),
                                        sqrt(m_Params.Roughness) * u_EnvRadianceTexLevels)
                               .rgb *
                             (1.0 - u_RadiancePrefilter);

    // Sample BRDF Lut, 1.0 - roughness for y-coord because texture was generated (in Sparky) for
    // gloss model
    vec2 specularBRDF =
      texture(u_BRDFLUTTexture, vec2(m_Params.NdotV, 1.0 - m_Params.Roughness)).rg;
...
void main()
{
    // Standard PBR inputs
    m_Params.Albedo =
      u_AlbedoTexToggle > 0.5 ? texture(u_AlbedoTexture, vs_Input.TexCoord).rgb : u_AlbedoColor;
    m_Params.Metalness =
      u_MetalnessTexToggle > 0.5 ? texture(u_MetalnessTexture, vs_Input.TexCoord).r : u_Metalness;
    m_Params.Roughness =
      u_RoughnessTexToggle > 0.5 ? texture(u_RoughnessTexture, vs_Input.TexCoord).r : u_Roughness;
    m_Params.Roughness =
      max(m_Params.Roughness, 0.05);    // Minimum roughness of 0.05 to keep specular highlight
